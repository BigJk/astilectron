			...
			case consts.eventNames.windowCmdMessageCallback:
				...
				if (targetElement)
					targetElement.webContents.send(
						json.name === consts.eventNames.windowCmdMessageCallback ? consts.eventNames.ipcCmdMessageCallback : consts.eventNames.ipcCmdMessage,
						m
					);
				break;
...
function windowCreate(json) {
	...
	if (typeof json.windowOptions.proxy !== 'undefined') {
		elements[json.targetID].webContents.session.setProxy(json.windowOptions.proxy).then(() => windowCreateFinish(json));
	} else {
...
function windowCreateFinish(json) {
	...
	elements[json.targetID].on('close', (e) => {
		if (typeof windowOptions[json.targetID] !== 'undefined' && typeof windowOptions[json.targetID].custom !== 'undefined') {
			if (typeof windowOptions[json.targetID].custom.messageBoxOnClose !== 'undefined') {
				...
				if (
					typeof windowOptions[json.targetID].custom.messageBoxOnClose.confirmId !== 'undefined' &&
					windowOptions[json.targetID].custom.messageBoxOnClose.confirmId !== buttonId
				) {
					e.preventDefault();
					return;
				}
	...
	elements[json.targetID].on('enter-full-screen', () => {
		client.write(json.targetID, consts.eventNames.windowEventEnterFullScreen, {
	...
	elements[json.targetID].on('leave-full-screen', () => {
		client.write(json.targetID, consts.eventNames.windowEventLeaveFullScreen, {
	...
	elements[json.targetID].on('moved', () => {
		let bounds = elements[json.targetID].getBounds();
		client.write(json.targetID, consts.eventNames.windowEventMoved, {
			bounds: bounds,
		});
	});
	...
	elements[json.targetID].on('will-move', () => {
		let bounds = elements[json.targetID].getBounds();
		client.write(json.targetID, consts.eventNames.windowEventWillMove, {
			bounds: bounds,
		});
	});
	elements[json.targetID].webContents.on('did-finish-load', () => {
		elements[json.targetID].webContents.executeJavaScript(
            ...
            ` +
				(typeof json.windowOptions.custom !== 'undefined' && typeof json.windowOptions.custom.script !== 'undefined'
